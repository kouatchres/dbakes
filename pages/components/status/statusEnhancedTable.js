import { useDeleteAnEmployeeStatusMutation } from '@/graphql';
...
import CreateEmployeeStatus from './createStatus';
import UpdateEmployeeStatus from './UpdateStatus';
...
const EmployeeStatusEnhancedTable = ({
  columns,
  data,
  isUpdatePopupOpen,
  isDeletePopupOpen,
  ...
  const [
    ...
    { data: dataEmployeeStatusDelete, loading, error }
  ] = useDeleteAnEmployeeStatusMutation();

  const handleDeleteEmployeeStatusConfirm = () => {
    deleteAEmployeeStatusMutation({
      variables: { where: { id: isDeletePopupOpen.id } }
    });

    setNotify({
      isOpen: true,
      message: 'Status sucessfully deleted',
      type: 'success'
    });
    handleDeletePopupChange();
  };
  ...
  return (
    <TableContainer style={{ paddingBottom: '1rem' }}>
      ...
      <MaUTable {...getTableProps()}>
        ...
        >
          <UpdateEmployeeStatus id={isUpdatePopupOpen.id} />
        </UpdatePopup>
      </MaUTable>
...
export default EmployeeStatusEnhancedTable;
